...
it("default after-close signals form exactly one bounded structured message per channel", async () => {
...
it("all native observations remain immutable in the ledger; exact point joins expose filtering and preserve aggregates", () => {
  ...
  expect(html).toContain("静默时段：转收盘汇总");
...
it("message samples are generated by the real renderers and stay reviewable", () => {
  ...
  if (process.env.P2_UPDATE_SAMPLES === "1")
    writeFileSync("docs/p2-message-samples.md", text);
  expect(readFileSync("docs/p2-message-samples.md", "utf8")).toBe(text);
});
