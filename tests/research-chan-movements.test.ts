...
import type { ChanMovement, CzscMovements } from "../src/lib/czsc-movements";
import {
  chanC4Presets,
  chanC4Trend,
  chanC4Sequence,
...
it("all final five methods remain planned/disabled until actual DLL verification", () => {
  ...
  expect(
    chanC4Presets.every(
      (p) =>
        p.status === "planned" &&
        !p.enabled &&
        p.validation === "awaiting-real-dll",
    ),
  ).toBe(true);
});
