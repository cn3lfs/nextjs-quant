...
import { evaluateFormula, futureFunctions } from "../../../src/lib/formula/tdx-formula";
import { arities } from "../../../src/lib/formula/tdx-formula-check";
import type { Bar } from "../../../src/lib/domain";
const bars = [1, 2, 3, 4].map((close, i): Bar => ({
  date: `2026-01-0${i + 1}`,
  open: close,
  high: close + 1,
  low: close - 1,
...
const additions: [string, string, number][] = [
  ["WMA", "WMA(C,3)", 20 / 6],
  ["DMA", "DMA(C,0.5)", 3.125],
  ["TMA", "TMA(C,0.5,0.25)", 1.625],
  ["HHVBARS", "HHVBARS(C,3)", 0],
  ["LLVBARS", "LLVBARS(C,3)", 2],
  ["BARSCOUNT", "BARSCOUNT(C)", 4],
  ["AVEDEV", "AVEDEV(C,3)", 2 / 3],
  ["SLOPE", "SLOPE(C,3)", 1],
  ["STDP", "STDP(C,3)", Math.sqrt(2 / 3)],
  ["VAR", "VAR(C,3)", 1],
  ["INTPART", "INTPART(-C/3)", -1],
  ["ROUND", "ROUND(-C+0.5)", -4],
  ["POW", "POW(C,2)", 16],
  ["SQRT", "SQRT(C)", 2],
  ["LOG", "LOG(C*25)", 2],
  ["MOD", "MOD(C,3)", 1],
  ["SGN", "SGN(-C)", -1],
  ["BETWEEN", "BETWEEN(C,4,2)", 1],
  ["VALUEWHEN", "VALUEWHEN(C<3,C)", 2],
  ["RANGE", "RANGE(C,2,4)", 0],
  ["IFF", "IFF(C>3,8,9)", 8],
  ["EVERY", "EVERY(C>1,3)", 1],
  ["IFN", "IFN(C>3,8,9)", 9],
  ["CEILING", "CEILING(-C/3)", -1],
  ["FLOOR", "FLOOR(-C/3)", -2],
  ["FRACPART", "FRACPART(-C/3)", -1 / 3],
  ["SIGN", "SIGN(C)", 1],
  ["EXP", "EXP(C-4)", 1],
  ["LN", "LN(C/4)", 0],
  ["SIN", "SIN(C-4)", 0],
...
describe("Q2b every new function", () => {
  ...
  it.each(additions)(
    "%s: hand-checkable value, causal prefix, determinism, immutable input and hard future gate",
    (_name, expr, expected) => {
      ...
      const extended = [
        ...input,
        { ...bars[0]!, date: "2026-01-05", close: 999 },
        { ...bars[0]!, date: "2026-01-06", close: 0 },
      ];
      expect(
        evaluateFormula(source, extended, params).outputs[0]!.values.slice(
          0,
          4,
        ),
      ).toEqual(a.outputs[0]!.values);
      ...
      for (const future of futureFunctions) {
        ...
        expect(() =>
          evaluateFormula(`坏:= ${future}(C,3,1);\n${source}`, input, params),
        ).toThrow(`第1行 ${future}`);
      }
      expect(() =>
        evaluateFormula(`${source}\n坏:=REF(C,-1);`, input, params),
      ).toThrow("第2行 REF");
    },
  );
  it("preserves full windows, population N=1, null domains and recursive gap state", () => {
    const out = (expr: string) =>
      evaluateFormula(expr, bars).outputs[0]!.values;
    ...
    for (const expr of [
      ...
      expect(out(expr)).toEqual([null, null, null, null]);
