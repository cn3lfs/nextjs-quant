...
it.skipIf(process.platform !== "win32")("loads the external downloader as UTF-8 and propagates its failure", () => {
  const root = mkdtempSync(resolve(tmpdir(), "quant-utf8-downloader-"));
  try {
    const script = resolve(root, "download.ps1");
    writeFileSync(script, 'param([switch]$MarketOnly)\n# 中文下载脚本，无BOM\nWrite-Output $MarketOnly.IsPresent\nexit 7\n', "utf8");
    const wrapper = readFileSync(resolve("scripts/workflow-task.ps1"), "utf8");
    const loader = wrapper.match(/\$loader = '([^']+)'/)?.[1];
    expect(loader).toBeTruthy();
    for (const enabled of ["1", "0"]) {
      const result = spawnSync("powershell.exe", ["-NoProfile", "-EncodedCommand", Buffer.from(loader!, "utf16le").toString("base64")], {
        encoding: "utf8", windowsHide: true, timeout: 15000,
        env: {...process.env, QUANT_TDX_DOWNLOADER: script, QUANT_TDX_MARKET_ONLY: enabled},
      });
