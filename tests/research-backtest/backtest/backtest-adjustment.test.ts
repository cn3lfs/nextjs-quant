...
import { equityDailyReturns } from "../../../src/lib/backtest/multiple-testing";
import { walkForwardSchema } from "../../../src/lib/backtest/walk-forward";
...
it("页面回显残余跳空、未建模边界，模式输入贯通API与worker", () => {
  ...
  expect(readFileSync("src/server/api/routers/jobs.ts", "utf8")).toContain(
    "input.adjustment",
  );
