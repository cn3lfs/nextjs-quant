...
it("reports registration absent from map, and claimed implementation without code/tests", () => {
  const unknown = {
    ...
    owners: new Map([
      ...evidence.owners,
      ["flag-unmapped", "src/lib/research/technical/research-channels.ts"],
    ]),
  };
...
it("backfills stale fields deterministically but never infers semantic completeness from a green test", () => {
  ...
  expect(fixed.implementation).toContain("src/lib/research/technical/research-channels.ts");
  expect(fixed.tests).toContain("tests/research-backtest/research-contracts.test.ts");
