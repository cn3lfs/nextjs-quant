...
it("adds contracts for strategy changes and rejects empty/traversing spot selections", () => {
  expect(spotTests(["tests/research-method.test.ts"])).toContain(
    "tests/research-contracts.test.ts",
  );
  expect(spotTests(["tests/core.test.ts"])).toEqual(["tests/core.test.ts"]);
...
it("selects contracts from actual shared runtime dependencies even when test names differ", () => {
  const graph = new Map([
    ...
    ["tests/research-contracts.test.ts", new Set(["src/server/run.ts"])],
