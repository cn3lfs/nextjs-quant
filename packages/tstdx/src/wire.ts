 ...
 * 兼容实现与真实抓包交叉核对后得到的协议事实。实现代码为本仓库自有；
 * 握手末字节的兼容性修正见 docs/2026-09-15/01-tstdx-handshake-fix.md。
 *
 ...
 * 沿用旧兼容握手，但 0x0FDB 报文末字节须为 0x05。
 * 2026-09-15 同节点单变量实测：0x02 仅有握手应答，K 线/五档无正文；
 * 改为 0x05 后旧查询恢复。字段正式语义未知，不引入客户端身份或账户数据。
 */
export const SETUP_FRAMES = [
  Buffer.from("0c0218930001030003000d0001", "hex"),
  Buffer.from("0c0218940001030003000d0002", "hex"),
  Buffer.from(
    "0c031899000120002000db0fd5d0c9ccd6a4a8af0000008fc22540130000d500c9ccbdf0d7ea00000005",
    "hex",
  ),
] as const;
...
export function splitSymbol(symbol: string) {
  if (typeof symbol !== "string" || !/^(sh|sz|bj)\d{6}$/.test(symbol))
    throw new Error("证券代码须为 sh/sz/bj 加六位数字");
  const market = MARKET[symbol.slice(0, 2) as keyof typeof MARKET];
