...
- [v-series-log.md](v-series-log.md)：V1–V5 六批的执行记录与逐条裁定。
- [v-series-walk-forward-run.md](v-series-walk-forward-run.md)：新尺子的第一次实际读数
  （sh600000 全历史、默认双均线、252/63）。含三条解读，其中「峰度 93.6 可能来自
  不复权除权跳空」是本次运行提出、**尚未核实**的候选问题，不在 V 系列任何一批范围内。
  这份记录属 A 层：数字是程序算的，未经人工复核。
- [w-prechecks-2026-09-13.md](w-prechecks-2026-09-13.md)：两项只读前置排查——
  信号台账为 0 条的原因（15:05 守卫从未满足，非故障）与峰度 93.6 的来源
  （确认为除权跳空，最极端五日全部落在 GBBQ 除权日且幅度超过涨跌停上限）。
  后者的结论是：不复权会污染 V1 的紧缩夏普，复权口径应排在策略研究之前。
- [w2-real-account-review-2026-09-13.md](w2-real-account-review-2026-09-13.md)：
  R/U 系列复盘工具第一次吃到真实交割单（2775 笔成交 / 190 条现金流 / 785 个回合）。
  结论：链路通了；账户负期望（胜率 50.89%、盈亏比 0.89、利润因子 0.53）；
  **百分比小亏而金额大亏的背离指向仓位管理**。两个已知缺口：TWR 因 70 笔出入金
  缺前值估值无法整体计算、执行损耗反事实链路留空。全程隔离目录，未写生产库。
