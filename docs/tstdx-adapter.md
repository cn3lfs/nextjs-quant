...
最新为 `tstdx-adapter-2`：ETF 分时/成交/五档按证券目录精度解码，指数盘口清空，未知报价时间返回 null 并保留原始编码；指数分时量不冒充股/手。10 个品种日/周/5 分钟 30 组通过，详情及未确认字段见[执行记录](data-source-execution.md)。握手及 adapter 基线已提交 `eb53365`；本轮精度修复尚未提交。
