...
- **测试**：[indicators.test.ts](../tests/engineering-validation/indicators.test.ts)、[q1-chart.test.ts](../tests/market-chart/chart/q1-chart.test.ts)、[q2a-formula.test.ts](../tests/screening-rps/rps/q2a-formula.test.ts)、[breakout.test.ts](../tests/strategy-signals/methods/breakout/breakout.test.ts)。全仓库禁止新增重复公式的静态扫描：**无测试保护**。
- **违反会怎样**：图上看到的指标与选股依据不一致，用户无法复核。

### U1 日收益绩效口径

- **是什么**：新增 17 项统一由 [daily-performance.ts](../src/lib/backtest/daily-performance.ts) 计算，通过复盘分段 `wbtStats` 追加；旧六项不替换。旧 `sharpe` 仍为减日无风险利率、样本标准差 ddof=1；新 `sharpeWbt` 固定 rf=0、总体标准差 ddof=0。新年化波动率使用全体有效收益的总体标准差，下行波动率使用负收益子集的总体标准差；旧 `sortino` 使用减 rf 的下偏二阶矩，分母为全部收益数，不能互换。
...
- **测试**：[daily-performance.test.ts](../tests/research-backtest/backtest/daily-performance.test.ts) 手算表、退化量、双曲线与年化参数；[trade-review-nav.test.ts](../tests/portfolio-ledger/trade-review-nav.test.ts) 修改前既有 R9 合成 fixture 的六项精确快照及首亏基线。真实账户复验由管理者执行。
- **违反会怎样**：同名夏普含义改变、缺失被伪装成零风险，或现有复盘数字漂移。

### U2 交易日回溯与矩阵缺失

- **是什么**：分段调用 U1 全部 17 项；A 股回溯由 [period-performance.ts](../src/lib/backtest/period-performance.ts) 的 `aSharePeriodTradingDays` 定义为 5/10/20/60/120/250 交易日，日历由调用方提供。`ytd` 从自然年首日对齐日历；不足窗口返回实际起止、交易日数和 `truncated`，有效收益不足 5 日标 `insufficientSample`。czscflow 用自然日，我们用交易日，故同名分段的数值不可与其输出直接比对。
...
- **测试**：[period-performance.test.ts](../tests/research-backtest/backtest/period-performance.test.ts)、[period-performance-service.test.ts](../tests/research-backtest/backtest/period-performance-service.test.ts)、[trade-review-api.test.ts](../tests/portfolio-ledger/trade-review-api.test.ts)。真实账户和浏览器由管理者验收。
...
- **测试**：[rolling-performance.test.ts](../tests/research-backtest/backtest/rolling-performance.test.ts)、[rolling-performance-service.test.ts](../tests/research-backtest/backtest/rolling-performance-service.test.ts)、[rolling-performance-api.test.ts](../tests/research-backtest/backtest/rolling-performance-api.test.ts)、[rolling-performance-chart.test.ts](../tests/research-backtest/backtest/rolling-performance-chart.test.ts) 覆盖切窗、门槛、wbt 反例、指标同源、分页、路由与断线。真实账户和浏览器交互尚未人工验收。
...
- **测试**：[position-risk.test.ts](../tests/portfolio-ledger/position-risk.test.ts)、[trade-review-nav.test.ts](../tests/portfolio-ledger/trade-review-nav.test.ts)、[position-risk-service.test.ts](../tests/portfolio-ledger/position-risk-service.test.ts)、[position-risk-api.test.ts](../tests/portfolio-ledger/position-risk-api.test.ts)、[position-risk-chart.test.ts](../tests/portfolio-ledger/position-risk-chart.test.ts) 保护手算、逆回购、缺失、原数值、分页及断线。
...
- **测试**：[holdings-correlation.test.ts](../tests/portfolio-ledger/holdings-correlation.test.ts)、[holdings-correlation-api.test.ts](../tests/portfolio-ledger/holdings-correlation-api.test.ts) 保护手算、19/20/21、缺失、数量不变性、逆回购、分页及路由。真实账户与浏览器尚未验收。
- **违反会怎样**：交易行为被误认为价格联动，缺失或小样本被误读为分散依据。

### U4 执行质量与反事实

- **是什么**：日 VWAP 只经 [trade-review-vwap.ts](../src/lib/portfolio/trade-review-vwap.ts) 的 `tradeReviewDayVwap` 计算，R12 价格除数校验与 U4 共用；基准具名为 `dayVwap`，不是委托价。成交量或成交额非正、非有限、缺唯一当日日线时留空并说明，不拿收盘价或原成交价替代。可得的一字板等不主观剔除，按成交额筛选。
...
- **测试**：[execution-quality.test.ts](../tests/research-backtest/execution-quality.test.ts) 覆盖四方向手算、金额加权、缺失传播、R12 转债、零损耗锚点、非零现金价差、边界不一致、脱敏和分页；真实账户与浏览器验收由管理者执行。
...
- **测试**：[execution-quality.test.ts](../tests/research-backtest/execution-quality.test.ts) 覆盖负值、零值、同边界零值、缺失日、正常 TWR 不变、服务端透传与页面原因；510 与 333 是修复前历史证据；当前真实样本按 W12 验收（需 `W12_REAL_DATA=1` 和本地原始文件），仍断言执行损耗及期末差不可用。
- **违反会怎样**：将失效反事实包装成执行损耗，或混淆非正净值与收益缺失证据。

### U8 策略准入判定

- **是什么**：判定只调用 [daily-performance.ts](../src/lib/backtest/daily-performance.ts) 的 `simple` + `annualRiskFreeRate=0`，年化因子走 U1 默认或显式 `yearlyDays` 并回显；展示净值曲线仍用复利。U1 的单利“仅作对照”在 U8 判定中例外，结果 `basis="simple"`，禁止第二份收益、波动率、回撤或夏普实现。
...
- **测试**：[strategy-admission.test.ts](../tests/strategy-signals/strategy-admission.test.ts)、[strategy-admission-service.test.ts](../tests/strategy-signals/strategy-admission-service.test.ts)。真实账户与浏览器由管理者验收；[合成分布](review/u8-calibration-distribution.json) 不代表账户阈值已标定。
...
- 测试：[three-segment-sample.test.ts](../tests/research-backtest/backtest/three-segment-sample.test.ts)。字段与版本差异及快照边界处理见 [decisions](decisions.md)。
...
- **测试**：[multiple-testing.test.ts](../tests/research-backtest/backtest/multiple-testing.test.ts) 保护分位点、手算、单调性、偏度惩罚、U1 同源、退化、矩阵、拼接和页面；[walk-forward.test.ts](../tests/research-backtest/backtest/walk-forward.test.ts) 固定接入前 summary 全部数值。真实浏览器及统计适用性人工验收尚未执行。
...
- **测试**：[backtest-overfit.test.ts](../tests/research-backtest/backtest/backtest-overfit.test.ts) 保护组合、双并列、缺失奇偶、退化和手算回归；[backtest-overfit-api.test.ts](../tests/research-backtest/backtest/backtest-overfit-api.test.ts) 保护普通响应剥离 λ 与按需完整导出；既有 walk-forward 数值护栏保持。
...
- **测试**：[research-usage.test.ts](../tests/research-backtest/research-usage.test.ts) 保护 321 条全量、重叠、重复运行、失败隔离、限额隔离、留出边界、文案与 DSR 不变；公式与独立 worker 的接入另由运行路径测试保护。真实浏览器由管理者验收。
...
- **测试**：[indicators.test.ts](../tests/engineering-validation/indicators.test.ts) 覆盖手算、空值、退化、参数与确定性；[q2a-formula.test.ts](../tests/screening-rps/rps/q2a-formula.test.ts) 覆盖序列基元；[q1-chart.test.ts](../tests/market-chart/chart/q1-chart.test.ts) 覆盖可调参数默认等价。与真实终端逐项人工一致：**无自动测试保护**，见 [M1 核对表](review/m1-tdx-checklist.md)。
- **违反会怎样**：BOLL 整体偏移、MACD 柱差一倍、历史窗口移动后指标漂移。

## 3. 未来函数必须硬拒绝

- **是什么**：[tdx-formula-check.ts](../src/lib/formula/tdx-formula-check.ts) 在求值前检查全部语句，含未使用赋值和死分支。命中以下 **31 项**即拒绝执行并给函数名/行号，不能“警告后继续”：
...
- **测试**：[q2a-formula.test.ts](../tests/screening-rps/rps/q2a-formula.test.ts)、[q2b-functions.test.ts](../tests/screening-rps/rps/q2b-functions.test.ts)、[q2b-screening.test.ts](../tests/screening-rps/rps/q2b-screening.test.ts)。
...
- **测试**：[q2a-formula.test.ts](../tests/screening-rps/rps/q2a-formula.test.ts)、[q2b-functions.test.ts](../tests/screening-rps/rps/q2b-functions.test.ts)；终端逐项对照**无自动测试保护**，见 [Q2b 核对](review/q2b-review/README.md)。
...
- **测试**：[indicators.test.ts](../tests/engineering-validation/indicators.test.ts)、[chart-data.test.ts](../tests/market-chart/chart/chart-data.test.ts)、[breakout.test.ts](../tests/strategy-signals/methods/breakout/breakout.test.ts)、[breakout-batch.test.ts](../tests/strategy-signals/methods/breakout/breakout-batch.test.ts)、[q2a-formula.test.ts](../tests/screening-rps/rps/q2a-formula.test.ts)、[q2b-functions.test.ts](../tests/screening-rps/rps/q2b-functions.test.ts)、[q2b-screening.test.ts](../tests/screening-rps/rps/q2b-screening.test.ts)、[signal-ledger.test.ts](../tests/strategy-signals/signals/signal-ledger.test.ts)。
...
- **测试**：[signal-ledger.test.ts](../tests/strategy-signals/signals/signal-ledger.test.ts) 覆盖三态、区间两端、收益、基线、调度、幂等和未来隔离；[p2-notification-policy.test.ts](../tests/strategy-signals/monitoring/p2-notification-policy.test.ts) 覆盖过滤不改台账。
...
- **测试**：[czsc.test.ts](../tests/strategy-signals/methods/chan/czsc.test.ts)、[m4-native.test.ts](../tests/engineering-validation/m4-native.test.ts)、[signal-ledger-worker.test.ts](../tests/strategy-signals/signals/signal-ledger-worker.test.ts)。跨独立应用实例只有一个 DLL 进程：**无测试保护，也非当前代码保证**。
...
- **测试**：[breakout.test.ts](../tests/strategy-signals/methods/breakout/breakout.test.ts)、[breakout-batch.test.ts](../tests/strategy-signals/methods/breakout/breakout-batch.test.ts)；阈值合理性需 [M5 人工确认](review/m5-review/README.md)，**无自动测试保护**。
...
- **测试**：[m4-monitor.test.ts](../tests/strategy-signals/monitoring/m4-monitor.test.ts)、[delivery-subscription.test.ts](../tests/research-backtest/delivery-subscription.test.ts)、[monitor-revision.test.ts](../tests/strategy-signals/monitoring/monitor-revision.test.ts)、[p2-notification-policy.test.ts](../tests/strategy-signals/monitoring/p2-notification-policy.test.ts)、[core.test.ts](../tests/persistence-infrastructure/core.test.ts)。假渠道端到端的**实际第三方投递数为 0**；受控本地 HTTP 不等于真实外发。
...
- **测试**：[p1-trade-ledger.test.ts](../tests/portfolio-ledger/p1-trade-ledger.test.ts)、[p1-trade-services.test.ts](../tests/portfolio-ledger/p1-trade-services.test.ts)。
...
- **测试**：[q1-chart.test.ts](../tests/market-chart/chart/q1-chart.test.ts)、[chart-data.test.ts](../tests/market-chart/chart/chart-data.test.ts)、[chart.test.ts](../tests/market-chart/chart/chart.test.ts)、[weekly-bars.test.ts](../tests/market-chart/market-data/weekly-bars.test.ts)。真实交互体验由用户确认。
...
- **测试**：[core.test.ts](../tests/persistence-infrastructure/core.test.ts)、[integration.test.ts](../tests/persistence-infrastructure/integration.test.ts)、[tdx-gbbq.test.ts](../tests/market-chart/market-data/tdx-gbbq.test.ts)、[tail-cache-codec.test.ts](../tests/engineering-validation/tail-cache-codec.test.ts)、[signal-ledger.test.ts](../tests/strategy-signals/signals/signal-ledger.test.ts)、[q1-chart.test.ts](../tests/market-chart/chart/q1-chart.test.ts) 覆盖解析、快照及迁移保留。setup 文件是隔离机制，其正确挂载/不指向生产、每次重打包纪律：**无专门测试保护**。
...
- **测试**：[signal-ledger-worker.test.ts](../tests/strategy-signals/signals/signal-ledger-worker.test.ts)、[q2b-screening.test.ts](../tests/screening-rps/rps/q2b-screening.test.ts)、[workbench-refactor.test.ts](../tests/workbench/workbench-refactor.test.ts)、[czsc.test.ts](../tests/strategy-signals/methods/chan/czsc.test.ts)。全市场 <10 分钟的持续自动门禁：**无测试保护**，历史测量见 review。
...
- **测试**：[research.test.ts](../tests/research-backtest/research.test.ts)、[report-security.test.ts](../tests/market-chart/market-data/report-security.test.ts)、[local-llm.test.ts](../tests/engineering-validation/local-llm.test.ts)、[research-skills.test.ts](../tests/research-backtest/research-skills.test.ts)、[m4-monitor.test.ts](../tests/strategy-signals/monitoring/m4-monitor.test.ts)。
...
- **是什么**：新增 [signal-information.ts](../src/lib/strategy-facts/signal-information.ts) 与旧聚合并存。按策略×方向×期限分组，short 的 returnPct 不取反；按观察日计算平均并列秩 Spearman，截面少于5条留空。IC样本标准差 ddof=1，小于10截面警告；不足2截面或标准差为零的比率留空。分层默认5组，每组期望少于10条降3组，仍不足整组留空；全样本经验分位数，并列跨界整体归低组，空组不填零。
- **为什么**：空头是价格方向观察，反号会虚构做空收益；拆并列与补零会制造评分差异。
- **测试**：[signal-information.test.ts](../tests/strategy-signals/signals/signal-information.test.ts) 覆盖手算、方向、排除、去重、门槛、分层、退化与服务端选页展示。
...
- **测试**：[universe-audit.test.ts](../tests/screening-rps/universe-audit.test.ts) 保护手算、边界、空池、零核验/外呼、只读名单、原快照成员、分页与下界展示。真实数据与浏览器验收尚待管理者执行。
...
- **测试**：[backtest-adjustment.test.ts](../tests/research-backtest/backtest/backtest-adjustment.test.ts) 保护手算净值、现金不入账、碎股、拒绝清单、默认完整结果哈希、信号反例、日期对齐与候选/fold透传。实际证券与浏览器验收由管理者执行，无自动化真实数据证明。
...
- **测试**：[trade-review-nav.test.ts](../tests/portfolio-ledger/trade-review-nav.test.ts) 保护修改前完整输出快照、三日手算、显式优先、缺口、空仓、逆回购与多笔反例；[trade-review-ui.test.ts](../tests/portfolio-ledger/trade-review-ui.test.ts) 保护两种模式的页面说明与重放输入。
...
- **测试**：[w8-repo-direction.test.ts](../tests/engineering-validation/w8-repo-direction.test.ts) 保护配平、跨品种不抵消、缺失与冲突留痕、普通证券不变；W8_REAL_DATA=1 启用本地真实账单验收及修改前非逆回购完整输出 SHA-256 护栏。
...
- **测试**：[w10-intraday-order.test.ts](../tests/strategy-signals/w10-intraday-order.test.ts) 保护手算、买卖双顺序的逐项回合/成本、跨日、输入不变、非法金额、余额锚点拒绝与标注；`W10_REAL_DATA=1` 核对指定本地文件的完整回合结果、逐日精确现金值及流水数量/金额。W8 非逆回购哈希护栏保持。
- **违反会怎样**：普通证券统计漂移、虚构现金改善、日末对账失真，或掩盖数据源排序假设。

### 账户对账与研究证据可信化（2026-09-14）

- 逐日现金对账只比较有来源证据的柜台余额与现有账本日末现金，不调整期初现金、分类、NAV 或成交排序；缺失、来源冲突和非法输入明确保留，JSON 可追溯到文件哈希与原始行号。
- VWAP 成交额加权均值沿用既有算法，算术均值单列；有效数量、金额覆盖和排除原因同时披露。诊断类别不是已证实根因，不放宽倍率、500 BP 或执行损耗不可用边界。
- 五类研究 attempt 与成功候选使用记录分开；失败、取消、宿主中断不计作成功候选。普通探索审计失败显示不完整，不伪造历史记录或试验总数。
- 样本最终验证冻结完整配置、数据、交易条件与运行引擎哈希后才能计算。揭示必须持久化成功才开放结果、衍生统计和导出；重试复用冻结输入并拒绝版本漂移，删除任务不抹去审计事实。
- 区间重叠提示仅说明可能被使用，不能证明样本从未被应用外研究触碰。该流程不证明历史证券池完整或策略业绩可信。
- 测试见 `cash-reconciliation.test.ts`、`research-governance.test.ts`、`research-lifecycle.test.ts`、`research-governance-api.test.ts` 及既有执行质量测试。
...
- **测试**：[execution-quality.test.ts](../tests/research-backtest/execution-quality.test.ts) 覆盖四条自检、同代码跨日期倍率、close 无关性、区间边界、未知倍率、异常双边排除及完整导出；真实验收保持回合 785 / 50.892857% / 0.891201。
