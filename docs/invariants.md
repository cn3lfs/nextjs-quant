...
- **是什么**：N1 台账与 Q2b 公式为 worker 批处理，不阻塞 UI/监控，支持取消/进度；全市场单次 <10 分钟为管理者验收口径，不再按交互选股的倍数要求优化。性能工作继续冻结。合法 UI 变化可更新结构指纹，但须单独精确验证新增部分，不删、不跳过护栏。DLL EBUSY 要定位原因，不接受“重跑就好”。
...
- **是什么**：M1–M5 已完成，不能重新展开交付清单。A 层机器验证与 B 层人工确认分开；N2/P3 等样本成熟再做；不增加新策略、不解冻模块、不自建回测、不优化性能。局部边界自行决定并记录；仅范围、架构、不可逆动作、许可事项升级。历史“不动某文件”与已授权交付冲突时以交付为准并记录，不能拿它越过当前用户限制。
- **为什么**：防止把已完成工作重做、把小样本/机器核对冒充有效性结论。GPL 对外捆绑决定仍交用户；这里只记录项目分发门禁，不作法律结论。
- **测试**：**无测试保护**（工程与授权纪律）。冻结范围见 [roadmap §2](roadmap.md#2-范围外冻结清单)，N2/P3 完整规格见 [next-plan.md](next-plan.md)。
- **违反会怎样**：范围失控、错误业绩宣传、未经授权分发；README“不能用作正式策略业绩”的限定必须永久保留。
