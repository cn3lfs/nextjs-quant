...
- **测试**：[execution-quality.test.ts](../tests/execution-quality.test.ts) 覆盖负值、零值、同边界零值、缺失日、正常 TWR 不变、服务端透传与页面原因；真实样本验收独立断言 510 与 333 且不相等（需 `W11_REAL_DATA=1` 和本地原始文件）。
