...
只输出 JSON，结构：
{
 "新闻要点": ["逐条提炼该板块原始新闻的关键信息，每条保留公司名/标的/具体数据/事件，覆盖主要新闻（条数随新闻量，一般3-10条）"],
...
- 最新交易日若为当日盘后初值，其绝对值可能明显小于前几日，**不得据此得出"资金已停止流出/流入"的结论**。
...
def flow_sub_line(rec, top=FLOW_SUB_TOP):
    ...
    return ' ｜ '.join(parts) if parts else '二级细分近5日净额均为 0'
...
def analyze_flows(flows, results):
    ...
    asof = next(iter(flows.values()))['数据截止']
    return call(FLOW_SYS, f"各板块主力资金净流入实测（截止 {asof}，单位亿元，正=净流入）：\n" + "\n".join(lines))
...
def main():
    ...
    if flows:
        ...
        md.append(f"> 实测数据，截止 **{next(iter(flows.values()))['数据截止']}**，来源：腾讯自选股"
                  ...
                  "最新交易日为当日盘后初值，绝对值可能偏小，方向判断以近5日/近20日为准。")
        md.append("")
        md.append("| 板块 | 最新日 | 近5日 | 近10日 | 近20日 | 二级细分（近5日，吸金 ↔ 失血） | 资金读法 |")
        md.append("|------|--------|-------|--------|--------|------------------------------|---------|")
        for name, _ in sectors:
            f = flows.get(name)
            if not f:
    ...
    for name, r in results:
        ...
        if sf:
            ...
            md.append(f"  - 资金流实测（{sf['数据截止']}，腾讯源）：主力净流入 最新日 {sf['最新日亿']:+.2f}亿"
