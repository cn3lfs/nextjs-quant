...
自用沪深量化工作台：股票与行业概念浏览、RPS强势池、缠论/双突破预选与确认、策略样本研究、财联社观点复盘。2026-09-10 用户授权扩充并修改历史冻结模块；E0–E4 后续交付及真实业务待验收见 [2026-09-11 工作流计划](2026-09-14/01-workflow-plan-2026-09-11.md)。程序正确不代表策略赚钱，不自动交易。
...
2026-09-15 用户要求把全局交易 skills 中每个有思路的方法做成策略，先详细规划、再分批实施，效果交给回测判断。范围、已有实现映射、完整性清点及 K0–K13 批次见[交易 skills 策略化计划](trading-skills-strategy-plan.md)。用户已授权实施，进度见[执行记录](2026-09-16/09-trading-skills-execution.md)；不因旧技能“只做报告”的交付边界遗漏方法，数据与交易正确性约束保持。

2026-09-15 后续数据源工作统一见[逐源适配与验收计划](archive/2026-09-22/data-sources/data-source-adaptation-plan.md)：补齐既有四个图表源与独立 mx-ds-mcp 查询的契约、正确性及验收，保持通达信文件只读和研究时点约束。

2026-09-14 用户授权临时停用积分耗尽的 tdx-finance MCP，保留实现；以免费四源及 UI 可选来源替代相关行情调用，优先级和当前可用性见 [数据源说明](data-sources.md)。

2026-09-11 用户授权执行通达信数据层优化方案，P0—P4进度与证据见[执行记录](2026-09-14/02-tdx-data-optimization-progress.md)。最新边界：分钟下载由用户和通达信负责，不开发客户端控制器；量化端负责读取、周期合成与完整性检查。日线恢复既有Windows全量下载脚本，增量调度停用；名称缺口通过公开资料逐项核实，不因缺名删减证券。
...
- CANSLIM、威科夫、基本面等技能按[策略化计划](trading-skills-strategy-plan.md)逐项规格化并补齐可运行策略，不按预想效果筛掉；自动交易、机器学习平台目前不排期。
