...
2026-09-15 用户要求把全局交易 skills 中每个有思路的方法做成策略，先详细规划、再分批实施，效果交给回测判断。范围、已有实现映射、完整性清点及 K0–K13 批次见[交易 skills 策略化计划](trading-skills-strategy-plan.md)。用户已授权实施，进度见[执行记录](trading-skills-execution.md)；不因旧技能“只做报告”的交付边界遗漏方法，数据与交易正确性约束保持。
