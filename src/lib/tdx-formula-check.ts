...
export function unary(op: string, a: number) {
  return op === "NOT" ? +(a === 0) : op === "-" ? -a : a;
}
...
const exact = (n: number): Range =>
  Number.isFinite(n)
    ? { min: n, max: n, integer: Number.isSafeInteger(n), constant: n }
    : unknown;
  ...
  for (const [name, value] of Object.entries(parameters)) {
    ...
    if (
      ...
      issues.push({
        ...
        name,
        kind: "parameter",
        reason: "参数须为唯一 N1/N2… 名称及有限数值",
      });
    } else {
      params.set(key, value);
      names.set(key, exact(value));
    }
  }
  const report = (
    expr: Expr,
    kind: FormulaIssue["kind"],
    reason: string,
    name?: string,
  ) => issues.push({ line: expr.line, name, kind, reason });
  function visit(e: Expr): Range {
    if (e.kind === "string") {
      report(e, "unsupported", "不支持字符串求值");
      return unknown;
    }
    ...
    if (e.kind === "unary") {
      const r = visit(e.value),
        n = scalar(r);
      if (n !== undefined) return exact(unary(e.op, n));
      return e.op === "NOT"
        ? truth
        : e.op === "-"
          ? { min: -r.max, max: -r.min, integer: r.integer }
          : r;
    }
    if (e.kind === "binary") {
      const a = visit(e.left),
        b = visit(e.right),
        x = scalar(a),
        y = scalar(b);
    ...
    if (!Object.hasOwn(arities, e.name)) {
      report(e, "unsupported", "函数不支持", e.name);
      return unknown;
    }
    if (args.length !== arities[e.name]) {
      report(
        e,
        "parameter",
        `需要 ${arities[e.name]} 个参数，实际 ${args.length} 个`,
        e.name,
      );
      return unknown;
    }
    if (e.name === "REF") {
      const n = args[1]!;
      if (!(n.min >= 0 && n.integer))
        report(
          e,
          "future",
          "REF 偏移为负数或无法静态证明为非负整数，拒绝执行",
          e.name,
        );
      return { ...args[0]!, constant: undefined };
    }
    const periodPositions =
      e.name === "SMA" || e.name === "LAST"
        ? [1, 2]
        : [
              "MA",
              "EMA",
    ...
    for (const pos of periodPositions) {
      ...
      const minimum = ["STD", "VAR", "SLOPE"].includes(e.name)
        ? 2
        : [
              "MA",
              "EMA",
      ...
      if (n === undefined || !Number.isSafeInteger(n) || n < minimum)
        report(
          e,
          "parameter",
          `第${pos + 1}参数须为静态整数且 >= ${minimum}`,
          e.name,
        );
    }
    if (e.name === "SMA" && scalar(args[2])! > scalar(args[1])!)
      report(e, "parameter", "SMA 要求 M <= N", e.name);
    if (
      e.name === "DMA" &&
      scalar(args[1]) !== undefined &&
      !(scalar(args[1])! > 0 && scalar(args[1])! < 1)
    )
      report(e, "parameter", "DMA 要求 0<A<1", e.name);
    if (e.name === "TMA")
      for (const pos of [1, 2]) {
        if (scalar(args[pos]) === undefined || scalar(args[pos])! >= 1)
          report(e, "parameter", "TMA 的 A/B 须为小于1的静态有限数值", e.name);
      }
    if (
      e.name === "LAST" &&
      scalar(args[1]) !== 0 &&
      scalar(args[1])! < scalar(args[2])!
    )
      report(
        e,
        "parameter",
        "LAST 要求 A >= B（A=0 表示从首个有效值）",
        e.name,
      );
    if (
      [
      ...
      return truth;
    if (
      ["COUNT", "BARSLAST", "HHVBARS", "LLVBARS", "BARSCOUNT"].includes(e.name)
    )
      return { min: 0, max: Infinity, integer: true };
    if (
      e.name === "IF" &&
      scalar(args[0]) !== undefined &&
      scalar(args[1]) !== undefined &&
      scalar(args[2]) !== undefined
    )
      return exact(scalar(args[0]) !== 0 ? scalar(args[1])! : scalar(args[2])!);
    if (e.name === "IF")
      return {
        min: Math.min(args[1]!.min, args[2]!.min),
        max: Math.max(args[1]!.max, args[2]!.max),
        integer: args[1]!.integer && args[2]!.integer,
      };
    if (e.name === "ABS") {
      const r = args[0]!,
        n = scalar(r);
      return n !== undefined
        ...
        : {
            min: 0,
            max: Math.max(Math.abs(r.min), Math.abs(r.max)),
            integer: r.integer,
          };
    }
    if (e.name === "MAX" || e.name === "MIN") {
      const fn = e.name === "MAX" ? Math.max : Math.min;
      if (scalar(args[0]) !== undefined && scalar(args[1]) !== undefined)
        return exact(fn(scalar(args[0])!, scalar(args[1])!));
      return {
        min: fn(args[0]!.min, args[1]!.min),
        max: fn(args[0]!.max, args[1]!.max),
        integer: args[0]!.integer && args[1]!.integer,
      };
    }
    return unknown;
  }
