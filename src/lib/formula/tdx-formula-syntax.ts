...
export class FormulaError extends Error {
  constructor(readonly issues: FormulaIssue[]) {
    super(
      issues
        .map((i) => `第${i.line}行${i.name ? ` ${i.name}` : ""}：${i.reason}`)
        .join("\n"),
    );
...
const fail = (line: number, reason: string): never => {
  throw new FormulaError([{ line, kind: "syntax", reason }]);
};
...
function lex(source: string): Token[] {
  ...
  while (i < source.length) {
    ...
    if (number) {
      ...
      tokens.push({ text: number, kind: "number", line });
      i += number.length;
      continue;
    }
    const name = /^[\p{L}_][\p{L}\p{N}_]*/u.exec(rest)?.[0];
    if (name) {
      tokens.push({ text: name.toUpperCase(), kind: "name", line });
      i += name.length;
      continue;
    }
    const symbol = /^(?::=|>=|<=|<>|!=|&&|\|\||[+\-*/><=():;,])/.exec(
    ...
    tokens.push({ text: symbol, kind: "symbol", line });
    i += symbol.length;
  }
...
export function parseFormula(source: string): Statement[] {
  ...
  const expect = (text: string) => {
    if (peek().text !== text)
      fail(peek().line, `缺少 ${text}，实际为 ${peek().text || "结尾"}`);
    take();
  };
  function expression(min = 0): Expr {
    ...
    if (token.kind === "number")
      left = { kind: "number", value: Number(token.text), line: token.line };
    else if (token.kind === "string")
      left = { kind: "string", value: token.text, line: token.line };
    else if (["+", "-", "NOT"].includes(token.text))
      left = {
        kind: "unary",
        op: token.text,
        value: expression(6),
      ...
      if (peek().text === "(") {
        ...
        expect(")");
        left = { kind: "call", name: token.text, args, line: token.line };
      } else left = { kind: "name", name: token.text, line: token.line };
    } else
      return fail(token.line, `需要表达式，实际为 ${token.text || "结尾"}`);
    while ((precedence[peek().text] ?? -1) >= min) {
      ...
      left = {
        ...
        op: op.text,
        left,
        right: expression(precedence[op.text]! + 1),
        line: op.line,
      };
    }
    depth--;
