...
export function saveFormula(input: unknown) {
  ...
  return put("tdx-formula", `tdx-formula-${id}`, {
    ...formula,
    id,
    updatedAt: Date.now(),
  });
...
export function formulaScreenJob(input: unknown) {
  ...
  return background(
    "screen",
    { ...work, period: "day", source: "tdx-local", adjustment: "none" },
    async (job, signal) => {
      ...
      return {
        ...
        universeSource: "当前本地通达信全 A 股日线",
        researchWarnings: [
          "使用最新已完成本地日线，不承诺数据为当前交易日；不复权。候选表均线/量比为既有默认描述指标，不参与公式判定。",
        ],
      };
    },
    work,
