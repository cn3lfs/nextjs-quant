...
export function NotificationPolicyFields({
  ...
  return (
    <fieldset>
      ...
      <p>
        可跨午夜，开始与结束相同表示全天静默。汇总仅在设定时间起15分钟内放行非交易时段静默，仍遵守自定义静默；错过窗口或封卷后迟到只记台账，不跨日补推。立即档绕过非交易时段静默，仍受自定义静默、每日限额和去重窗口约束。日线下立即档与汇总档实际相差约10分钟。去重按每渠道的首次投递尝试计，重试不增加逻辑消息数。
      </p>
      <p>
